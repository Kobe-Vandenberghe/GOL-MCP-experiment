"""Turning WorldService results into the text the agent and the activity log read.

Pure functions: dataclass in, string out. Kept apart from the tools so the
phrasing of an operation lives in one place regardless of which surface asked
for it.
"""
...
def status_text(st: WorldStatus, viewers: int) -> str:
    box_txt = (
    ...
    return (
        ...
        f"Connected browser viewers: {viewers}\n"
        f"World revision: {st.revision} (bumped by every change, from any source)"
    )


def timeline_status_text(t: TimelineStatus) -> str:
    return (
        f"Timeline status\n"
...
def create_text(r: CreateResult) -> str:
    return (
...
def create_log(r: CreateResult) -> str:
...
def set_cells_text(r: SetCellsResult) -> str:
    msg = (
...
def placement_text(r: PlacementResult) -> str:
    msg = (
...
def advance_text(r: AdvanceResult) -> str:
    return (
...
def rewind_text(r: RewindResult) -> str:
    return (
...
def sim_summary_text(summary: dict) -> str:
    bits = [
...
def sim_run_json(run: SimulationRun) -> str:
    if run.failed:
...
def sim_interrupted_log(name: str, run: SimulationRun) -> str:
    return (
