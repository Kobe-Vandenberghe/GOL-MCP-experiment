...
const st = { width: 0, height: 0, generation: 0, population: 0,
             running: false, fps: 10, edge: "wrap" };
...
const ALIVE = pack(0x7c, 0xe3, 0x8b);
...
function applyState(m) {
  ...
  const raw = atob(m.cells);
  const n = st.width * st.height;
  for (let i = 0; i < n; i++) {
    buf32[i] = (raw.charCodeAt(i >> 3) >> (7 - (i & 7))) & 1 ? ALIVE : DEAD;
  }
...
$("btn-step").onclick = () => send({ action: "step" });
$("btn-clear").onclick = () => send({ action: "clear" });
